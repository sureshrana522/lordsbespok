    ...
    <div className="min-h-screen bg-black text-white font-sans">
         ...
         {selectedOrder && (
            <div className="space-y-6">
                        ...
                        {MEASUREMENT_FIELDS[item.type]?.map(field => (
                           <div key={field}>
                              ...
                              <input 
                                 ...
                                 value={measurementData[idx]?.[field] || ''}
                                 onChange={(e) => {
               ...
               <div className="flex gap-2 pt-2">
                  <button onClick={() => handleSaveMeasurement(false)} className="flex-1 bg-zinc-800 text-gray-300 py-3 rounded">Save Draft</button>
                  <button onClick={() => handleSaveMeasurement(true)} className="flex-1 bg-gold-600 text-black font-bold py-3 rounded">Complete & Handover</button>
               </div>
            </div>
         )}
      </Modal>

      <Modal isOpen={activeModal === 'CUTTING_SELECT'} onClose={() => setActiveModal(null)} title="Select Cutting Master">
          <div className="space-y-2">
             {users.filter(u => u.role === Role.CUTTING).map(cutter => (
                <button 
                   key={cutter.id}
                   onClick={() => setSelectedCuttingMaster(cutter.id)}
                   className={`w-full text-left p-3 rounded border ${selectedCuttingMaster === cutter.id ? 'bg-gold-900/30 border-gold-500 text-white' : 'bg-black border-zinc-800 text-gray-400'}`}
                >
                   {cutter.name}
                </button>
             ))}
             <button onClick={confirmHandover} disabled={!selectedCuttingMaster} className="w-full bg-gold-600 text-black font-bold py-3 rounded mt-4 disabled:opacity-50">Confirm Handover</button>
          </div>
      </Modal>
      ...
      <Modal isOpen={activeModal === 'WALLET'} onClose={() => setActiveModal(null)} title="My Complete Wallet">
          <div className="space-y-1">
            {[
              { id: 'stitchingWallet', name: 'Stitching Wallet', balance: currentUser.wallet.stitchingWallet },
          ...
          <div className="bg-black border border-zinc-800 p-4 rounded-lg mt-4">
             <h4 className="text-white text-sm mb-3">Add Money to Wallet</h4>
             <input placeholder="Amount (₹)" type="number" className="w-full bg-zinc-900 border border-zinc-700 p-2 rounded text-white mb-2" value={paymentForm.amount} onChange={e => setPaymentForm({...paymentForm, amount: e.target.value})} />
             <input placeholder="UTR / Ref Number" className="w-full bg-zinc-900 border border-zinc-700 p-2 rounded text-white mb-2" value={paymentForm.utr} onChange={e => setPaymentForm({...paymentForm, utr: e.target.value})} />
             <button onClick={handleAddMoney} className="w-full bg-green-600 text-black font-bold py-2 rounded hover:bg-green-500">Deposit Funds</button>
          </div>
          ...
          <div className="space-y-2">
             {inProgressOrders.map(o => (
                 <div key={o.id} className="bg-zinc-900 p-3 rounded flex justify-between items-center">
                    <span className="text-sm text-white">{o.billNumber} - {o.customerName}</span>
                    <button onClick={() => handleOpenMeasurement(o)} className="text-xs bg-gold-900/20 text-gold-400 px-3 py-1 rounded">Resume</button>
                 </div>
             ))}
          </div>
      </Modal>
      ...
      <HelpAgent userRole="Measurement Master" userName={currentUser.name} />
